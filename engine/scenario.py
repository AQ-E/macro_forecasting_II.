...
class ScenarioEngine:
    ...
    def run_scenario(self, horizon, targets):
        ...
        for h in range(1, horizon + 1):
            ...
            cur_base = self.step_macro(cur_base, targets, is_scenario=False, h=h-1)
            cur_scen = self.step_macro(cur_scen, targets, is_scenario=True, h=h-1)
    ...
    def step_macro(self, row, targets, is_scenario=True, h=0):
        new_row = row.copy()
        ...
        if is_scenario:
            # Handle Path vs Scalar targets
            def get_target(key, default):
                val = targets.get(key, default)
                if isinstance(val, (list, np.ndarray, pd.Series)):
                    return val[h] if h < len(val) else val.iloc[-1] if hasattr(val, 'iloc') else val[-1]
                return val

            # Shift log_gdp
            g_rate = get_target('gdp_growth', 0.108)
            new_row['log_gdp'] += math.log(max(1 + g_rate, 0.001))
            new_row['log_gdp_hat'] = new_row['log_gdp']
            
            new_row['inflation'] = get_target('inflation', row['inflation'])
            new_row['policy rate'] = get_target('policy_rate', row['policy rate'])
            
            # exrate propagation
            new_row['exrate'] *= (1 + get_target('exrate_growth', 0.0))
